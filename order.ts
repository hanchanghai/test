...
import { createOrderByConfig, updateOrderByConfig, getMyOrderList, getMyOrder, getMyOrderStatus, updateOrderByLoan, giveUpGracePeriod, getMyOrderByPomsid, getPagingOrderList } from "../service/MyOrderService";
import { simulatePreOrderByOrderId, simulatePreOrderByConfigId, simulateGetOrder, simulateGetPayments, simulatePaymentRecord, simulateGetOrderByPomsid, simulateVinNotification, simulatePomsSns } from "../service/SimulateService";
import {checkout, requestRefund, updateOrderDetail, updateOrderGroup} from "../service/CheckoutService";
import { estimateDiscount, initOrderDiscount } from "../service/PromoService";
...
router.get("/", (req, res) => {
    let pomsid: string = req.query.pomsid as string;

    let pageIndex: string = req.query.pageIndex as string;
    let itemsPerPage: string = req.query.itemsPerPage as string;
...
router.get("/detail", (req, res) => {
    extractTokenInfoWithoutVerify(req)
        .then(token => getOrderDetailByPomsId(token, req.query.pomsid as string))
        .then(result => res.status(200).jsonp(result))
        .catch(e => {
            handleServiceException(e, res);
        });
...
router.get("/loan", (req, res) => {
    extractTokenInfoWithoutVerify(req)
        .then(token => getOrderLoanInfoByPomsId(token, req.query.pomsid as string))
        .then(result => res.status(200).jsonp(result))
        .catch(e => {
            handleServiceException(e, res);
        });
...
router.get('/vehicle', async (req, res, next) => {
    extractTokenInfoWithoutVerify(req)
        .then(token => getCompletedVehicle(req.query.pscnid as string || token.id as string))
